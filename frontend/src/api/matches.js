...
/**
 * Server-side Gemini recap for a completed match (teams, venue, result, notes).
 * @param {string} id Match id
 * @returns {Promise<{ summary: string; model?: string }>}
 */
export async function getMatchAiSummary(id) {
  const { data } = await api.get(`/matches/${id}/ai-summary`, { timeout: 90_000 });
  return data;
}
